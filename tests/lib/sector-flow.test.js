...
(async () => {
  ...
  /* ⚠️ 방향성 단언은 제거했다. 참이 아닌 명제였다.
   *
   * 2026-08-18 에 "정정 − 사이트 = T·(1/d − 1) 이므로 오늘값 부호가 방향을 정한다"는
   * 규칙을 세워 테스트에 박았다. 그 대수는 **양쪽이 같은 과거 스냅샷을 쓴다**는 전제에서만
   * 성립하는데, 실제로는 기준일부터 다르다 (2026-08-19 실측):
   *     정정값  10일 기준 = 2026-08-03 (평일만 세어 진짜 10거래일 전)
   *     사이트  10 인덱스 = 2026-08-08 (주말 스냅샷이 섞인 사이트 방식)
   * 즉 두 값은 ①공식(파이썬 vs 브라우저) ②감쇠 적용 여부 ③기준 날짜 세 가지가 모두 달라,
   * 어떤 단일 변수의 부호로도 대소를 예측할 수 없다.
   * (그 전 판본의 ">60% 는 사이트가 더 낮다"도 마찬가지로 그날 데이터의 우연이었다.)
   *
   * 대신 아래 두 가지만 검사한다 — 이건 구성상 참이다.
   *   · 사이트 재현값과 정정값이 실제로 다르다 (위 테스트, 버그가 존재한다는 증거)
   *   · 부호가 뒤집히는 업종이 존재한다 (위 테스트)
   * 우리 계산 자체의 정확성은 wrs.test.js 의 "사이트와 오차 0" 골든 테스트가 지킨다. */
  ok('사이트 재현값과 정정값의 기준일이 실제로 다르다 (섞어 쓰면 안 된다)', () => {
    // 이 차이가 사라지면 위 주석의 설명이 무효가 되므로, 전제 자체를 고정해 둔다.
    assert.notStrictEqual(d10, s10, '평일 기준일과 사이트 인덱스 기준일이 같아졌다 — 주석 재검토 필요');
    console.log(`     → 정정 ${d10} vs 사이트 ${s10} (25일: ${d25} vs ${s25})`);
  });
